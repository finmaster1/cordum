...
func main() {
	...
	req := &pb.JobRequest{
		...
		ContextPtr: ctxPtr,
		AdapterId:  "chat-advanced",
	}

	packet := &pb.BusPacket{
		TraceId:         traceID,
		SenderId:        "chat-advanced-sender",
	...
	if err := natsBus.Publish("sys.job.submit", packet); err != nil {
		log.Fatalf("failed to publish chat advanced job: %v", err)
	}
	log.Printf("sent chat advanced job job_id=%s trace_id=%s context_ptr=%s", jobID, traceID, ctxPtr)
}
