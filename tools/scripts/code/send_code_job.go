...
func main() {
	...
	if err := natsBus.Publish("sys.job.submit", packet); err != nil {
		log.Fatalf("failed to publish code job: %v", err)
	}
	log.Printf("sent code llm job job_id=%s trace_id=%s context_ptr=%s", jobID, traceID, ctxPtr)
}
