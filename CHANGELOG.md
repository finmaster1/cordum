...
- CI preview today: EDGE-143.2 has shipped only RED tests (no `detector.go` yet) and EDGE-143.3 is at 0/13 steps. The flag therefore prints a clear actionable `provider <name> not supported in this build; EDGE-143.2/.3 detector(s) must DONE first` for each of the 5 supported providers and exits non-zero. The `edgeDoctorCIHTTPTransport http.RoundTripper` seam is in place for `TestEdgeDoctor_ShadowCI_ReadOnly` to assert no POST / PUT / PATCH / DELETE is ever issued once the detector modules land. Unsupported providers (e.g. `azuredevops`) print `provider <X> not recognized; supported: github/gitlab/jenkins/buildkite/circleci`; malformed specs (missing colon) print `--shadow-ci value must be in <provider>:<token-or-config-path> format`.
