...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: Dict[str, Any] = {
        "method": "post",
        "url": "/api/v1/edge/shadow-agents/{finding_id}/suppress".format(
...
def sync_detailed(
    ...
    *,
    ...
    """Suppress a shadow-agent finding
     ...
     Flips a detected finding to suppressed. Idempotent on a suppressed finding; returns 409 when called
    on a resolved terminal record. Optional `suppressed_until` records a time-bound exception for
    downstream re-evaluation. Emits `shadow_agent.suppressed` audit event on success.
