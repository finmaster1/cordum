    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <div className="text-xs text-muted-foreground">Visualize distributed job execution traces</div>
        </CardHeader>
      ...
      {jobs.length > 0 ? (
        <Card>
          <CardHeader>
            ...
            <div className="text-xs text-muted-foreground">
              {jobs.length} spans · Started {formatRelative(new Date(epochToMillis(jobs[0].updated_at)).toISOString())}
            </div>
          </CardHeader>
            ...
            {jobs.map((job, index) => {
              ...
              return (
                <div key={job.id || index} className="relative group">
                  <div 
                    className="flex items-center gap-3 p-2 rounded-2xl hover:bg-card/50 border border-transparent hover:border-border transition-colors"
                  ...
                  >
                    ...
                    <div className="min-w-[140px] text-xs font-mono text-muted-foreground">
                       +{offset}ms
                    </div>
        ...
        <div className="p-4 text-sm text-muted-foreground">Trace found but contains no jobs (or access denied).</div>
      ) : null}
    </div>
  );
}
