    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <Card>
        <CardHeader>
          ...
          <div className="text-xs text-muted-foreground">View artifact content by pointer</div>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="text-xs text-muted-foreground">Manually store content</div>
        </CardHeader>
        <div className="space-y-3">
          <div>
            <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Content Type</label>
            <Input value={contentType} onChange={(e) => setContentType(e.target.value)} />
          </div>
          <div>
            <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Content</label>
            <Textarea 
              rows={8} 
              value={uploadContent} 
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <Card>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Resource Key</label>
            <Input value={resource} onChange={(e) => setResource(e.target.value)} placeholder="my-resource-lock" />
          </div>
          <div>
            <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Owner ID</label>
            <Input value={owner} onChange={(e) => setOwner(e.target.value)} />
          </div>
          <div>
            <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">TTL (ms)</label>
      ...
      <Card>
        ...
        {getQuery.isError ? (
          <div className="text-sm text-muted-foreground">Lock is currently free (not found).</div>
        ) : null}
        {getQuery.data ? (
          <div className="space-y-3 rounded-2xl border border-border bg-card/70 p-4">
           ...
           <div className="text-sm text-muted-foreground">Enter a resource key to check status.</div>
        )}
      </Card>
    </div>
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <div className="text-xs text-muted-foreground">Inspect raw Redis keys or Cordum pointers (ctx:*, res:*)</div>
      </CardHeader>
      <div className="flex gap-2">
      ...
      <div className="mt-6">
        {getQuery.isLoading ? <div className="text-sm text-muted-foreground">Loading...</div> : null}
        {getQuery.isError ? <div className="text-sm text-danger">Not found or invalid key.</div> : null}
        {getQuery.data ? (
          <div className="space-y-4">
            <div className="grid gap-4 lg:grid-cols-3">
              <div className="rounded-2xl border border-border bg-card/70 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Kind</div>
            ...
            <div>
              <div className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Content</div>
              <Textarea 
                readOnly 
                rows={16} 
                value={
