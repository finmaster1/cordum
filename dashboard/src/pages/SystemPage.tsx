...
export default function SystemPage() {
  ...
  const dlqQuery = useInfiniteQuery<Awaited<ReturnType<typeof api.listDLQPage>>>({
    queryKey: ["dlq"],
    queryFn: ({ pageParam }) =>
      api.listDLQPage(
        100,
  ...
  const dlqEntries = useMemo<RawDLQEntry[]>(
    () =>
      (dlqQuery.data?.pages.flatMap((page) => page.items ?? []) ?? []).map((entry) => ({
        ...
        attempts: entry.attempts ?? entry.retryCount,
        created_at: entry.createdAt || entry.failedAt || new Date().toISOString(),
      })),
    [dlqQuery.data]
  );
        ...
        <>
          <Card>
            <CardHeader>
              ...
              <div className="text-xs text-muted-foreground">Gateway status snapshot</div>
            </CardHeader>
            <div className="grid gap-4 lg:grid-cols-4">
              <div className="rounded-2xl border border-border bg-card/70 p-4">
                <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">NATS</div>
                <div className="mt-2 text-sm font-semibold text-ink">{String(nats?.status || "unknown")}</div>
                <div className="text-xs text-muted-foreground">{String(nats?.url || "-")}</div>
              </div>
              <div className="rounded-2xl border border-border bg-card/70 p-4">
                <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Redis</div>
                <div className="mt-2 text-sm font-semibold text-ink">{redis?.ok ? "ok" : "unavailable"}</div>
                <div className="text-xs text-muted-foreground">{String(redis?.error || "-")}</div>
              </div>
              <div className="rounded-2xl border border-border bg-card/70 p-4">
                <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Workers</div>
                <div className="mt-2 text-sm font-semibold text-ink">{workersCount ?? workers.length}</div>
                <div className="text-xs text-muted-foreground">Active worker heartbeats</div>
              </div>
              <div className="rounded-2xl border border-border bg-card/70 p-4">
          ...
          <Card>
            <CardHeader>
              ...
              <div className="text-xs text-muted-foreground">What needs triage right now</div>
            </CardHeader>
            <div className="grid gap-3 lg:grid-cols-3">
              <div className="list-row border-l-4 border-warning">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Stale workers</div>
              ...
              <div className="list-row border-l-4 border-danger">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">DLQ backlog</div>
                ...
                <div className="mt-2 text-xs text-muted-foreground">Retry or purge stuck jobs</div>
              </div>

              <div className="list-row border-l-4 border-danger">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Hot pools</div>
                ...
                <div className="mt-2 text-xs text-muted-foreground">Pools above 80% CPU or memory</div>
              </div>
            </div>
          </Card>
            ...
            {poolMetrics.length === 0 ? (
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                {poolMetrics.map((pool) => (
                  <div key={pool.name} className="rounded-2xl border border-border bg-card/70 p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-sm font-semibold text-ink">{pool.name}</div>
                        <div className="text-xs text-muted-foreground">{pool.topics} topics · {pool.workers} workers</div>
                      </div>
          ...
          {workers.length === 0 ? (
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              {workers.map((worker, index) => (
                <div key={`${worker.worker_id}-${index}`} className="rounded-2xl border border-border bg-card/70 p-4">
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-semibold text-ink">{worker.worker_id || "worker"}</div>
                    <Badge variant="info">{worker.pool || "default"}</Badge>
                  </div>
                  <div className="mt-2 text-xs text-muted-foreground">CPU {worker.cpu_load ?? "-"}%</div>
                  <div className="text-xs text-muted-foreground">Memory {worker.memory_load ?? "-"}%</div>
                </div>
              ))}
            </div>
          )}
                  ...
                  return (
                    <div key={entry.job_id} className="list-row">
                      <div className="grid gap-3 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_auto] lg:items-center">
                        ...
                        <div className="text-xs text-muted-foreground">Created {formatRelative(entry.created_at)}</div>
                      ...
                      {guidance ? (
                        <div
                          className={`mt-3 rounded-2xl border p-3 ${getGuidanceSeverityBg(guidance.severity)}`}
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <div className="text-sm font-medium text-ink">{guidance.title}</div>
                              <div className="mt-1 text-xs text-muted-foreground">{guidance.description}</div>
                            </div>
                            {guidance.action ? (
      ...
      {activeTab === "config" && (
        <>
          <Card>
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              <div>
                <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Scope</label>
              ...
              <div>
                <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Scope ID</label>
                <Input
                  value={configScopeId}
          ...
          <Card>
            <CardHeader>
              ...
              <div className="text-xs text-muted-foreground">Current scope vs system default</div>
            </CardHeader>
            <div className="grid gap-4 lg:grid-cols-2">
              <div>
                <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">System default</label>
                <Textarea readOnly rows={10} value={baseConfigText} />
              </div>
              <div>
                <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Current scope</label>
            ...
            {schemasQuery.data?.length ? (
              <div className="space-y-3">
                {(schemasQuery.data as Record<string, unknown>[]).map((schema, index) => (
                  <div key={`schema-${index}`} className="rounded-2xl border border-border bg-card/70 p-4">
        ...
        <>
          <Card>
            <CardHeader>
              ...
              <div className="text-xs text-muted-foreground">OpenTelemetry export and dashboards</div>
            </CardHeader>
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-2xl border border-border bg-card/70 p-4">
                <div className="flex items-center justify-between text-xs uppercase tracking-[0.2em] text-muted-foreground">
                ...
                <div className="text-xs text-muted-foreground">Protocol {otelProtocol.toUpperCase()}</div>
              </div>
              <div className="rounded-2xl border border-border bg-card/70 p-4">
                <div className="flex items-center justify-between text-xs uppercase tracking-[0.2em] text-muted-foreground">
                  <span>Grafana</span>
          ...
          <Card>
            <CardHeader>
              ...
              <div className="text-xs text-muted-foreground">Configure where telemetry is sent</div>
            </CardHeader>
            <div className="space-y-4">
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                <div>
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Endpoint</label>
                  <Input
                    value={otelEndpoint}
                ...
                <div>
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Protocol</label>
                  <Select
                    value={otelProtocol}
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div>
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Headers (JSON)</label>
                  <Textarea
                    rows={6}
                ...
                <div>
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Resource Attributes (JSON)</label>
                  <Textarea
                    rows={6}
              ...
              {!canEditConfig ? <div className="text-xs text-muted-foreground">Admin role required to update system config.</div> : null}
              <Button
                variant="primary"
                size="sm"
                type="button"
          ...
          <Card>
            <CardHeader>
              ...
              <div className="text-xs text-muted-foreground">Link external observability views</div>
            </CardHeader>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Base URL</label>
                <Input
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-2xl border border-border bg-card/70 p-4">
                  ...
                  <div className="text-xs text-muted-foreground">CPU, Memory, and NATS message rates.</div>
                  <Input
                    className="mt-3"
                ...
                <div className="rounded-2xl border border-border bg-card/70 p-4">
                  ...
                  <div className="text-xs text-muted-foreground">Latency, success rates, and cost per run.</div>
                  <Input
                    className="mt-3"
        ...
        <>
          <Card>
            <CardHeader>
              ...
              <div className="text-xs text-muted-foreground">Route critical events to on-call systems</div>
            </CardHeader>
            <div className="space-y-4">
              <div className="rounded-2xl border border-border bg-card/70 p-4">
